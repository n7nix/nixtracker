...
$(function () {
	...
	var outboxBtn = $('#outboxBtn');
        ...
        connection.onopen = function () {
                ...
                mailstatus.text('web socket status:');
                mailinput.val('Connection opened.');
        };

        connection.onerror = function (error) {
                // just in case there were problems with conenction...
                mailinput.val('Problem with connection or the server is down.');
        };
        ...
        connection.onmessage = function (message) {
                ...
                try {
                        ...
                        debugWin.append('MAIL: This doesn\'t look like a valid JSON: type: ' + message.type + " data: " + message.data);
...
//                        var json2 = json.data.replace(/\\n/g, '<BR>');
                        ...
                        debugWin.append('<p>MAIL: Unhandled: ' + 'name: ' + json.type + '</p>' );
	...
	document.getElementById("sendMheardBtn").onclick=function(){
		debugWin.append('<p>mheard</p>');
		/* Need to do this to get the last character typed */
		wl2kargsin.keydown();
		connection.send(JSON.stringify( { type: 'wl2kargs', data: wl2kargs} ));

		var msg = 'send_button_mheard';
		connection.send(JSON.stringify( { type: 'message', data: msg} ));
	};
        ...
        setInterval(function() {
                if (connection.readyState !== 1) {
                        ...
                        mailinput.attr('disabled', 'disabled').val('Unable to communicate '
                                + 'with the PLU WebSocket server.');
                }
        }, 3000);
